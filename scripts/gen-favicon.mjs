...
async function main() {
	...
	for (const target of pngTargets) {
		...
		if (target.file) {
			...
			console.log(
				`✓ PNG ${target.size}×${target.size} écrit : ${target.file} (${data.length} octets)`,
			);
		}
		if (target.forIco) {
	...
	console.log(
		`✓ ICO multi-résolution écrit : ${icoPath} (${icoData.length} octets, ${icoBuffers.length} images)`,
	);
	...
	console.log(
		`  favicon.ico                  (${icoData.length} o)  → 16+32+48 px (depuis logo source)`,
	);
